report 60003 "Customer Summary Ageing New"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Report/Layouts/CustomerSummaryAgeingNew.rdl';
    ApplicationArea = all;
    Caption = 'Customer Summary Ageing New';
    ...
    dataset
    {
        dataitem(Customer; Customer)
        {
            ...
            RequestFilterFields = "No.", "Global Dimension 1 Filter", "Customer Posting Group";
            ...
            column(Customer_TABLECAPTION__________Cust1Filter; Customer.TABLECAPTION + ': ' + Cust1Filter)
            {
            }
            ...
            column(Customer___Summary_AgeingCaption; Customer___Summary_AgeingCaptionLbl)
            {
            }
            ...
            begin
                ...
                FOR i := 1 TO 8 DO BEGIN
                    //DIM
                    IF Cust1Filter <> '' THEN
                        CustLedgEntry.SETFILTER(CustLedgEntry."Global Dimension 1 Code", Cust1Filter);
                    //DIM

                    IF DocumentDate THEN BEGIN
                        CustLedgEntry.SETCURRENTKEY("Customer No.", "Global Dimension 1 Code", "Document Date", Open);
                        CustLedgEntry.SETRANGE("Customer No.", "No.");
                        CustLedgEntry.SETFILTER("Global Dimension 1 Code", Cust1Filter);
                        CustLedgEntry.SETRANGE("Document Date", PeriodStartDate[i], PeriodStartDate[i + 1] - 1);
                        ...
                        CustBalanceDueLCY1[i] := Customer."Balance (LCY)";
                        LineTotalCustBalance := LineTotalCustBalance + CustBalanceDueLCY[i];
                    END;

                    IF PostingDate THEN BEGIN
                        CustLedgEntry.SETCURRENTKEY("Customer No.", "Global Dimension 1 Code", "Posting Date", Open);
                        CustLedgEntry.SETRANGE("Customer No.", "No.");
                        CustLedgEntry.SETFILTER("Global Dimension 1 Code", Cust1Filter);
                        CustLedgEntry.SETRANGE(Open, TRUE);
                        CustLedgEntry.SETRANGE("Posting Date", PeriodStartDate[i], PeriodStartDate[i + 1] - 1);
                        IF CustLedgEntry.FINDFIRST THEN
                            REPEAT
                                DtldCustLedgEntry.SETCURRENTKEY("Customer No.", "Posting Date", "Initial Entry Global Dim. 1");
                                DtldCustLedgEntry.SETRANGE("Customer No.", CustLedgEntry."Customer No.");
                                DtldCustLedgEntry.SETRANGE(DtldCustLedgEntry."Cust. Ledger Entry No.", CustLedgEntry."Entry No.");
                                DtldCustLedgEntry.SETFILTER("Posting Date", '<=%1', StartDate);
                                IF DtldCustLedgEntry.FINDFIRST THEN
                    ...
                    IF DueDate THEN BEGIN
                        CustLedgEntry.SETCURRENTKEY("Customer No.", "Global Dimension 1 Code", "Due Date", Open);
                        CustLedgEntry.SETRANGE("Customer No.", "No.");
                        CustLedgEntry.SETFILTER("Global Dimension 1 Code", Cust1Filter);
                        ...
                        IF CustLedgEntry.FINDFIRST THEN
                            REPEAT
                                DtldCustLedgEntry.SETCURRENTKEY("Customer No.", "Initial Entry Due Date", "Initial Entry Global Dim. 1");
                                DtldCustLedgEntry.SETRANGE("Customer No.", CustLedgEntry."Customer No.");
                                DtldCustLedgEntry.SETRANGE(DtldCustLedgEntry."Cust. Ledger Entry No.", CustLedgEntry."Entry No.");
                                DtldCustLedgEntry.SETFILTER("Posting Date", '<=%1', StartDate);
                                IF DtldCustLedgEntry.FINDFIRST THEN
            ...
            var
                Error1: Label 'The start date must be provided.';
    ...
    begin
        ...
        Cust1Filter := Customer.GETFILTER(Customer."Global Dimension 1 Filter");
    ...
    var
        ...
        Customer___Summary_AgeingCaptionLbl: Label 'Customer - Summary Ageing';
