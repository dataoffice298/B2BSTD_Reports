report 50001 "Purchase Order GST Viz"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Report/Layouts/PurchaseOrderGSTVizOrder.rdl';
    // RDLCLayout = 'PurchaseOrderGSTVizOrder1.rdl';
    Caption = 'Purchase Order GST';
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            column(GstNumberCap; GstNumberCap)
            { }
            column(PurchaseOrdDateCap; PurchaseOrdDateCap)
            { }
            column(PurchaseOrdNoCap; PurchaseOrdNoCap)
            { }
            column(YourQuotationNoCap; YourQuotationNoCap)
            { }
            column(YourQuotationDtCap; YourQuotationDtCap)
            { }
            column(TaxationCap; TaxationCap)
            { }
            column(PaymentCap; PaymentCap)
            { }
            column(InsuranceCap; InsuranceCap)
            { }
            column(FreightCap; FreightCap)
            { }
            ...
            { }
            column(TransporterCap; TransporterCap)
            { }
            column(SrCap; SrCap)
            { }
            column(HsnCap; HsnCap)
            { }
            column(TotalQtyCap; TotalQtyCap)
            { }
            column(ScheQtyCap; ScheQtyCap)
            { }
            column(RateCap; RateCap)
            { }
            column(TaxableValueCap; TaxableValueCap)
            { }
            column(RateAmtCap; RateAmtCap)
            { }
            column(TotalIncTaxLbl; TotalIncTaxLbl)
            { }
            column(TotalAmtCap; TotalAmtCap)
            { }
            column(UnitCap; UnitCap)
            { }
            column(SchDateCap; SchDateCap)
            { }
            ...
            { }
            column(OrderAmtCap; OrderAmtCap)
            { }
            column(AuthorisedCap; AuthorisedCap)
            { }
            column(StateCodeCap; StateCodeCap)
            { }
            column(GstinidCap; GstinidCap)
            { }
            ...
            { }
            column(TaxableCap; TaxableCap)
            { }
            column(IgstPerCap; IgstPerCap)
            { }
            column(IgstAmtCap; IgstAmtCap)
            { }
            column(CgstperCap; CgstperCap)
            { }
            column(CgstAmtCap; CgstAmtCap)
            { }
            column(SgstPerCap; SgstPerCap)
            { }
            column(SgstAmtCap; SgstAmtCap)
            { }
            ...
            dataitem(CopyLoop; Integer)
            {
                ...
                dataitem(PageLoop; Integer)
                {
                    ...
                    //Ch
                    column(Vendor_StateCode; Vendor."State Code")
                    {
                    }
                    //ch
                    ...
                    column(GstNo_PurchaseHeader; "Purchase Header"."Location GST Reg. No.")
                    {
                    }
                    ...
                    dataitem(RoundLoop; Integer)
                    {
                        ...
                        column(Expected_Receipt_Date; "Purchase Line"."Expected Receipt Date")
                        {

                        }
                        column(HSNCode_PurchaseLine; "Purchase Line"."HSN/SAC Code")
                        {
                        }
                        ...
                        //ch24
                        column(TotalTaxableAmtGvar; TotalTaxableAmtGvar)
                        { }
                        column(AmountInWords; AmountInWords[1])
                        { }
                        ...
                        begin
                            ...
                            if "Purchase Line"."No." <> GLAccountNo then begin
                                ...
                                TotalAmount += "Purchase Line"."Line Amount";


                            end;
                            //CHB2B24JUL23<<

                            TotalTaxableAmtGvar += "Purchase Line"."Line Amount" + Abs(SGSTAmt) + Abs(CGSTAmt) + Abs(IGSSTAmt) + Abs(0.00) + Abs(CessAmount) - TDSAmt;
                            CheckReportGre.InitTextVariable();
                            CheckReportGre.FormatNoText(AmountInWords, TotalTaxableAmtGvar, "Purchase Header"."Currency Code");
                        end;
    ...
    var
        ...
        //ch
        //CheckReportGre: codeunit "Check Codeunit";
        CheckReportGre: report "Voucher Register";

        AmountInWords: array[2] of Text[80];
        TaxableValueGvar: Decimal;
        TotalTaxableValue: Decimal;
        IGSTLbl: Label 'IGST';
        SGSTLbl: Label 'SGST';
        CGSTLbl: Label 'CGST';
        GstNumberCap: Label 'GST Number';
        PurchaseOrdNoCap: Label 'Purchase Ord No';
        PurchaseOrdDateCap: label 'Purchase Ord Date';
        YourQuotationNoCap: label 'Your Quotation No';
        YourQuotationDtCap: label 'Your Quotation Dt';
        VizTechonologiesPrivateCap: label 'VIZ Technologies Private Limited';
        StateCodeCap: Label 'State Code:';
        GstinidCap: Label 'GSTIN ID';
        TaxationCap: Label 'Taxation';
        PaymentCap: Label 'Payment';
        InsuranceCap: Label 'Insurance';
        TrasporterCap: Label 'Trasporter';
        FreightCap: Label 'Freight';
        DeliveryCap: Label 'Delivery';
        WarrantyCap: Label 'Warranty';
        SrCap: Label 'Sr';
        HsnCap: label 'HSN';
        TotalQtyCap: Label 'Qty';
        ScheQtyCap: Label 'Sche.Qty';
        UnitCap: Label 'Unit';
        SchDateCap: Label 'Sch.Date';
        RateCap: Label 'Rate';
        TaxableValueCap: Label 'Amount';
        RateAmtCap: Label 'Rate/Amt.';
        TotalAmt: Label 'Total Amt';
        Text000: label '1.Kindly mention the P.O. number and product Code in your Invoice.';
        Text001: label '2.Please deliver as per above schedule.';
        Text002: label '3.Inovice must be supplied with each delivery';
        Text003: Label '4.Please attach test certificate & yourself inspection report along with your Invoice.';
        Text004: label '5.Please give advance intimation in writing in case of inability of material as per mentioned schedule and tentative date of supply.';
        Text005: label '6.Weights/Measurements ascertained at our stores will be final';
        Text006: label '7.Materials are subject to our final approvals';
        Text007: label '8.This is computer -generated PO hence no signature required';
        Text008: label 'Subject to Rajkot jurisdication.';
        Text009: Label 'For SILVER CONSUMER ELECTRICALS PRIVATE LIMITED';

        Text010: label 'Purchase Order';
        AuthorisedCap: Label 'Authorised Signatory';
        OrderAmtCap: Label 'Order Amt';
        TransporterCap: label 'Trasporter';
        TotalAmtCap: label 'Total Amt';
        PartNoCap: label 'Part No.';
        TaxableCap: Label 'Taxable';
        IgstPerCap: label 'IGST%';
        IgstAmtCap: Label 'IGST AMT';
        CgstperCap: label 'CGST%';
        CgstAmtCap: Label 'CGST AMT';
        SgstPerCap: Label 'SGST%';
        SgstAmtCap: Label 'SGST AMT';
        TotalTaxableAmtGvar: Decimal;
        SGSTPer: decimal;
        CGSTPer: decimal;
        IGSTPer: decimal;
